...
const getConfig = (inputs, state) => {
    ...
    config.domain = inputs.domain
        ...
        : null
    config.nakedDomain = config.domain ? config.domain : null
...
const ensureCertificate = async (clients, config, instance) => {
    ...
    if (certificate.Status === 'PENDING_VALIDATION') {
        ...
        // it will timeout and the user will need to recreate and validate the certificate manually
        log(
            `Certificate validation timed out after 72 hours. Please recreate and validate the certificate manually.`
        )
